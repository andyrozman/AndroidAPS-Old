...
class OverviewFragment : DaggerFragment(), View.OnClickListener, OnLongClickListener {
    ...
    fun updateGUI(from: String) {
        ...
        if (Config.APS && pump.pumpDescription.isTempBasalCapable) {
            ...
            when {
                loopPlugin.isEnabled(PluginType.LOOP) && loopPlugin.isSuperBolus -> {
                    ...
                    overview_apsmode?.setBackgroundColor(getBackgroundColor(OverviewColorScheme.APS_SuperBolus))
                    overview_apsmode?.setTextColor(getTextColor(OverviewColorScheme.APS_SuperBolus))
                }

                loopPlugin.isDisconnected                                        -> {
                    overview_apsmode?.text = String.format(resourceHelper.gs(R.string.loopdisconnectedfor), loopPlugin.minutesToEndOfSuspend())
                    overview_apsmode?.setBackgroundColor(getBackgroundColor(OverviewColorScheme.APS_Loop_Disconnected))
                    overview_apsmode?.setTextColor(getTextColor(OverviewColorScheme.APS_Loop_Disconnected))
                }
                ...
                loopPlugin.isEnabled(PluginType.LOOP) && loopPlugin.isSuspended  -> {
                    ...
                    overview_apsmode?.setBackgroundColor(getBackgroundColor(OverviewColorScheme.APS_Loop_Suspended))
                    overview_apsmode?.setTextColor(getTextColor(OverviewColorScheme.APS_Loop_Suspended))
                }

                pump.isSuspended                                                 -> {
                    overview_apsmode?.text = resourceHelper.gs(R.string.pumpsuspended)
                    overview_apsmode?.setBackgroundColor(getBackgroundColor(OverviewColorScheme.APS_Pump_Suspended))
                    overview_apsmode?.setTextColor(getTextColor(OverviewColorScheme.APS_Pump_Suspended))
                }
                ...
                loopPlugin.isEnabled(PluginType.LOOP)                            -> {
                    ...
                    overview_apsmode?.setBackgroundColor(if (isLGS) resourceHelper.gc(R.color.ribbonUnusual) else getBackgroundColor(OverviewColorScheme.APS_Loop_Enabled))
                    overview_apsmode?.setTextColor(getTextColor(OverviewColorScheme.APS_Loop_Enabled))
                }

                else                                                             -> {
                    overview_apsmode?.text = resourceHelper.gs(R.string.disabledloop)
                    overview_apsmode?.setBackgroundColor(getBackgroundColor(OverviewColorScheme.APS_Loop_Disabled))
                    overview_apsmode?.setTextColor(getTextColor(OverviewColorScheme.APS_Loop_Disabled))
                }
            }
        } else {
        ...
        if (tempTarget != null) {
            ...
            overview_temptarget?.setTextColor(getTextColor(OverviewColorScheme.TempTargetNotSet))
